...
import com.tibco.as.spacebar.core.AS;
import com.tibco.as.spacebar.core.MetaType.ListOfMeta.Meta;
import com.tibco.as.spacebar.core.ObjectFactory;
import com.tibco.as.spacebar.core.Profile;
import com.tibco.as.spacebar.core.Profiles;
...
public class SpaceBarPlugin extends AbstractUIPlugin {
	...
	public static final String ID_PLUGIN = "com.tibco.as.spacebar.ui";
	...
	public Metaspaces getMetaspaces() {
		if (metaspaces == null) {
			...
			for (Profile profile : getProfiles()) {
				...
				metaspace.setAutoconnect(profile.isAutoconnect());
				metaspace.setDiscovery(profile.getDiscovery());
				metaspace.setListen(profile.getListen());
				metaspace.setMemberName(profile.getMember());
				metaspace.setMetaspaceName(profile.getMetaspace());
				metaspace.setRemote(profile.isRemote());
				metaspace.setTimeout(profile.getTimeout());
				metaspaces.addChild(metaspace);
					...
					for (Property property : properties) {
						if ("label".equals(property.getDataType().getValue())) {
							...
							if (AS_212_SPACEDEF_PROPERTIES.containsKey(id)) {
								if (!ASUtils.hasMethod(SpaceDef.class,
	...
	private List<Profile> getProfiles() {
		if (profiles == null) {
			...
			if (file.exists()) {
				try {
					profiles = (Profiles) JAXBContext
							.newInstance(Profiles.class).createUnmarshaller()
							.unmarshal(file);
				} catch (JAXBException e) {
					logException(
							NLS.bind(
				...
				profiles = objectFactory.createProfiles();
				Profile profile = objectFactory.createProfile();
				profile.setProfileName("Default");
				profile.setTimeout(30000);
				try {
					String hostName = InetAddress.getLocalHost().getHostName();
					profile.setMember("SpaceBar-" + hostName);
	...
	private File getProfilesFile() {
		return getStateLocation().append("profiles.xml").toFile();
	}
	...
	public void delete(Metaspace metaspace) {
		...
		for (Profile profile : list.toArray(new Profile[list.size()])) {
			if (profile.getProfileName().equals(metaspace.getName())) {
				list.remove(profile);
			}
		}
		saveProfiles();
	...
	private void saveProfiles() {
		File file = getProfilesFile();
		try {
			JAXBContext.newInstance(Profiles.class).createMarshaller()
					.marshal(profiles, file);
		} catch (JAXBException e) {
			logException(
	...
	public void modify(String oldName, Metaspace metaspace) {
		int index = getIndex(oldName);
		if (index == -1) {
			return;
		}
		getProfiles().set(index, getProfile(metaspace));
		saveProfiles();
	}
	 ...
	 * 
	 * @param profile
	 *            the profile to search for
	 * @return index of profile, or -1 if none found by that name
	 */
	private int getIndex(String name) {
		List<Profile> list = getProfiles();
		for (int index = 0; index < list.size(); index++) {
			if (list.get(index).getProfileName().equals(name)) {
				return index;
			}
		}
		return -1;
	...
	public void add(Metaspace metaspace) {
		getProfiles().add(getProfile(metaspace));
		saveProfiles();
	}

	private Profile getProfile(Metaspace metaspace) {
		Profile profile = objectFactory.createProfile();
		profile.setAutoconnect(metaspace.isAutoconnect());
		profile.setDiscovery(metaspace.getDiscovery());
		profile.setListen(metaspace.getListen());
		profile.setMember(metaspace.getMemberName());
		profile.setMetaspace(metaspace.getMetaspaceName());
		profile.setProfileName(metaspace.getName());
		profile.setRemote(metaspace.isRemote());
		profile.setTimeout(metaspace.getTimeout());
		return profile;
	}
