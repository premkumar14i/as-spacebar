...
public class IndexListEditor extends Composite {
	...
	private void remove() {
		...
		for (Object element : selection.toArray()) {
			indexes.removeChild(((Index) element).getName());
		}
		...
		Realm.runWithDefault(SWTObservables.getRealm(display), new Runnable() {
			public void run() {
				Space space = new Space(new Spaces(new Metaspace(
						new Metaspaces(), null), "Spaces"), SpaceDef
						.create("space1"));
				...
				Field field3 = new Field(fields, "Field3");
				...
				Indexes indexes = new Indexes(space, "Indexes");
