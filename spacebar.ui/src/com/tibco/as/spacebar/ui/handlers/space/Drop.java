...
public class Drop extends AbstractSpaceHandler {

	@Override
	protected void handle(ExecutionEvent event, final Space space) {
		Shell shell = HandlerUtil.getActiveShell(event);
		MessageDialog dialog = new MessageDialog(shell, "Confirm Space Drop",
				null, NLS.bind("Are you sure you want to drop space ''{0}''?",
						space), MessageDialog.QUESTION,
				new String[] { IDialogConstants.YES_LABEL,
						IDialogConstants.NO_LABEL }, 0); // yes is the default
		int code = dialog.open();
		if (code == MessageDialog.OK) {
			new UIJob(shell.getDisplay(), "DropSpace") {

				@Override
				public IStatus runInUIThread(IProgressMonitor monitor) {
					try {
						monitor.beginTask(
								NLS.bind("Dropping space ''{0}''", space), 1);
